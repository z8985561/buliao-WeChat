...
userinfo: function (options) {
  ...
  //选择规格事件
  chooseOption(e){
    var { index: isActive,src} = e.currentTarget.dataset;
    if (this.data.colorPic.length){
      this.setData({
        isActive:999,
        colorPic: []
      })
  ...
  //输入色号
  inputColorNumber(e){
    var colorNumber = e.detail.value;
    this.setData({ colorNumber })
  },
  //搜索颜色
  searchColor(){
    var id = this.data.goods.id;
    var colorList = [];
    var colorNumber = this.data.colorNumber;
    this.data.pickerOption.specs[0].items.forEach(item=>{
      item.title.indexOf(colorNumber) > -1 && colorList.push(item)
    })
